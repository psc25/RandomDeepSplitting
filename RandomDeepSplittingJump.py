...
class RDSPL_model():
    def __init__(self, T, N, M, d, x0, mu, sigmadiag, eta, Zdistr, M0, nuAdelta, f, g, hid_layer_size, activation, nu):
        self.T = T
        self.N = N
        self.dt = T/N
        self.M = M
        ...
        for t in range(N):
            ...
            Pmax = np.max(P[:, t])
            if Pmax > 0:
                Z = Zdistr.rvs(size = (M, Pmax, d)).astype(dtype = np.float32)
            ...
            V = Zdistr.rvs(size = (1, M0, d)).astype(dtype = np.float32)
